...
/* W1 message consists of a set of commands designated to be sent on the 1-wire
   ...
   specific slave device (the slave message). The master message constitutes of
   ...
   required data exchange); whereas the slave message constitutes only of FUNC
...
/* Initialize empty w1 master/slave message to store 'max_cmds' w1 command at max.
   The caller must allocate w1_msg_t struct before the call; use
...
/* Set master/slave id for w1 message.
   'p_id' must point to w1_master_id_t or w1_slave_id_t depending on message
   type (specified during w1 message initialization). The w1 message type can not
   be changed once the message has been initialized, so id's type must be always
   the same for the same w1 message.
 */
...
/*
   Add w1 command to the w1 message for execution by w1_msg_exec() in order
...
/* Execute w1 message. Data to be sent/retrieve are taken/written into the
   ...
   execution time of all command contained within single w1 message.
...
/* Initialize W1 handle
   ...
   NOTE: The initiated is associated with a netlink socket, therefore it's
   feasible to have multiple w1 handlers for various logically separate
   operations.
 */
...
/* Get id list of W1 bus masters and write it under 'p_masts'. The caller must
   allocate w1_masters_t struct (use get_w1_masters_bufsz() for this case) and
   set its 'max_sz' appropriately. If not NULL 'p_n_masts' will get overall
...
/* Search slave devices connected to a bus controlled by a 'master'. 'p_slavs'
   and 'p_n_slavs' have similar meaning as 'p_masts' and 'p_n_masts' for
   list_w1_masters().
 */
lr_errc_t search_w1_slaves(w1_hndl_t *p_hndl,
    w1_master_id_t master, w1_slaves_t *p_slavs, size_t *p_n_slavs);

/* Search slaves with alarms being set. Meaning of params is the same as for
   list_w1_slaves().
 */
